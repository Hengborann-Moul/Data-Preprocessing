...
# Augmentation Multipliers based on (Boredom, Engagement, Confusion, Frustration)
# Target for minority classes: ~600-800 samples total
COMBINATION_MULTIPLIERS = {
    # -- Majority (>1000) --
    (0, 1, 0, 0): 0,  # ~5982 -> Target: 5982 (No change)
    (1, 1, 0, 0): 0,  # ~1530 -> Target: 1530 (No change)
    (0, 1, 1, 0): 1,  # ~417  -> Target: ~834 (1x aug)
    # -- Moderate (100-300) --
    (1, 0, 0, 0): 2,  # ~258  -> Target: ~774 (2x aug)
    (1, 1, 1, 0): 3,  # ~192  -> Target: ~768 (3x aug)
    (0, 0, 0, 0): 4,  # ~127  -> Target: ~635 (4x aug)
    (0, 1, 0, 1): 5,  # ~113  -> Target: ~678 (5x aug)
    # -- Rare (<100) --
    (1, 1, 1, 1): 6,  # ~93   -> Target: ~650 (6x aug)
    (1, 1, 0, 1): 7,  # ~84   -> Target: ~672 (7x aug)
    (0, 1, 1, 1): 10,  # ~56   -> Target: ~616 (10x aug)
    (1, 0, 1, 0): 15,  # ~39   -> Target: ~624 (15x aug)
    (1, 0, 0, 1): 15,  # ~36   -> Target: ~576 (15x aug)
    (1, 0, 1, 1): 20,  # ~24   -> Target: ~500 (20x aug)
    (0, 0, 1, 0): 20,  # ~14   -> Target: ~300 (20x aug)
    (0, 0, 0, 1): 20,  # ~11   -> Target: ~240 (20x aug)
    (0, 0, 1, 1): 20,  # ~10   -> Target: ~220 (20x aug)
}


def get_multiplier(row):
    # Create tuple key: (B, E, C, F)
    key = (
        int(row["Boredom"]),
        int(row["Engagement"]),
        int(row["Confusion"]),
    ...
    # Return specific multiplier or 0 for unknown
    return COMBINATION_MULTIPLIERS.get(key, 0)
